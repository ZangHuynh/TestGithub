...
public class GUI_Sach extends JFrame implements ActionListener, MouseListener{
		...
		if(obj.equals(cbMaSach)) {
			...
			if(row != -1) {
			 ...
			 if(validData()) {
				 ...
				 if(bookList.capNhatSach(maSach, newBook)) {
	...
	private boolean validData() {
		...
		if(tacGia.isEmpty() && !tacGia.matches("([A-Z][a-z]*)+( [A-Z][a-z]*)+")) {
		...
		if(nhaXB.isEmpty()) {
			JOptionPane.showMessageDialog(null, "Error: Nhà xuất bản không được để rỗng");
		}
		...
		if(iSBN.isEmpty() && !iSBN.matches("\\d{1,4}(-\\d{1,4}){3,4}")) {
			JOptionPane.showMessageDialog(this, 
